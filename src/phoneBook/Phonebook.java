...
//            else if(!contact.getPhoneNUmber().equals(input) || !contact.getName().equals(input)){
//                display("Delete operation not successful");}
        }
    }
...
//                display("Contact not found");}
    ...
    private void checkInputIsANumber(String input){
        for(Character character:input.toCharArray()){
            if (!Character.isDigit(character)){
                throw new IllegalArgumentException("enter digts");
            }
        }
    }
    private boolean isA(String input){
        int no = Integer.parseInt(input);
        if(Integer.parseInt(input)==no){return true;}
        return false;
    }
